...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *outPtr=new double[indlen];
	...
	//plhs[2]=mxCreateDoubleMatrix(indlen,1, mxREAL);
	//double *outPtr=mxGetPr(plhs[2]);
...
//						}
	...
	out1Ptr[0]=outPtr[0]; out2Ptr[0]=1;
	for (i=1; i<indlen; i++) {
		if (outPtr[i]<out1Ptr[0]) {
			out1Ptr[0]=outPtr[i];
			out2Ptr[0]=i+1;
		}
	}
