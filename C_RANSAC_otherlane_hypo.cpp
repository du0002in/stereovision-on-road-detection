    ...
    for(j=0;j<300;j++)
    {
		...
		if (rand0<ratiol2r) //select points from left image
		{
			...
			*(u+1)=*(Datapointl+2*rand1+1);
			GetModel(v,u,&cos_Phai_H,D_pre_ptr,&A,&Bli,&Cli,&sign,L1L2);
		}
		else //select points from right image
		{
			...
			*(u+1)=*(Datapointr+2*rand1+1);
			GetModel(v,u,&cos_Phai_H,D_pre_ptr,&A,&Bri,&Cri,&sign,L1L2);
		}

		double Bli2=Bli+cos_Phai_H*L1L2[0]*sign;
		double Cli2=Cli-cos_Phai_H_D*L1L2[1]*sign; //ul=A/(vl-D)+Bli2*vl+Cli2;
		double Eli2=Cli2-Bli2**D_pre_ptr, Fli2=A-Cli2**D_pre_ptr;
		double Bri2=Bri+cos_Phai_H*L1L2[0]*sign;
		double Cri2=Cri-cos_Phai_H_D*L1L2[1]*sign;
