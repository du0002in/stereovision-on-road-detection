...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(j=0;j<600;j++)
    {
		...
		*u=*(Datapointl+2*rand1+1);
		...
		double L;
		L=(u[0]-u[1]-(-eli*v[0]+eri*v[1]-0.5*fli+0.5*fri))/(v[0]-v[1])/cos_Phai_H/sign;
		double eli2=eli-cos_Phai_H*L*sign;
		double fli2=(-eli2*v[0]-u[0])*2.0;
		double eri2=eri-cos_Phai_H*L*sign;
		double fri2=(-eri2*v[1]-u[1])*2.0;
