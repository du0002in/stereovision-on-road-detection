...
pre_U=zeros(2,15);
[U,~]=GA_Solver(X, ini_xc, ini_d_theta, abc, 0, Ts, pre_U, 0,lane_width); horizon=length(U);
...
%     phi2=phi1; phi3=phi2; v2=v1; v3=v2;   
    ...
    GAt=tic;
    [U,logposition(i),logw(i,:)]=GA_Solver(X, xc, d_theta, abc, .6*(phi1-phi2),loop_t, pre_U, k,lane_width);
    GAt_e=toc(GAt);
    logGAt(i)=GAt_e;
    S=[v2 v1 v1+cumsum(U(1,:))];
    V_t=(S(3:end)*deno_v+S(1:end-2)-(2+2.064*Ts)*S(2:end-1))/(7.82*Ts*Ts);
