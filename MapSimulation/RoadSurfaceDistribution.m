...
%for collection6
% log_ite_s=[1    555 1110 1725]; 
% log_ite_e=[370  960 1545 2040];

%for intensity distribution
log_ite_s=[575 686 740 804 1103 1317];
log_ite_e=[620 700 755 860 1248 1527];


for outloop=1:length(log_ite_s)
...
%     imwrite(dumy,strcat(save_dir,'\Ridge',int2str(ii),'.jpg'),'jpg');
    ...
    if ~isempty(xtmpl2)
        [pixX,pixY]=meshgrid([1:640],[1:240]);
% % % % %         To generate road surface intensity distribution
% % % %         if (xtmpl2(end)-xtmpl(end))>0
% % % %             xtmp_rl=xtmpl2-40;
% % % %             xtmp_ll=xtmpl+40;
% % % %             ytmp_rl=ytmpl2;
% % % %             ytmp_ll=ytmpl;
...
% % % %             ytmp_ll=ytmpl2;
% % % %         end
% % % %         [~,~,strict]=InPolygon(pixX(:),pixY(:),[xtmp_ll'; flipud(xtmp_rl')],-[ytmp_ll'; flipud(ytmp_rl')]);
% % % %         ind_intensity=sub2ind([240,640],pixY(strict==1),pixX(strict==1));
% % % %         intensity_value=img(ind_intensity);
% % % %         n(ii,:)=hist(intensity_value,0:1:255);
% % % % %       To generate lane line marking intensity distribution
% % % %         tdx_1=[ones(1,40),tdx];
% % % %         tdx_1=tdx_1(1:2:end);
% % % %         ytmpl_ctr=ytmpl;
% % % %         ind_ytmpl_ctr=(ytmpl_ctr<=-100)&(ytmpl_ctr>=-230);
% % % %         ytmpl_ctr=ytmpl_ctr(ind_ytmpl_ctr);
% % % %         tdx_1=tdx_1(-ytmpl_ctr);
% % % %         lane_lb=xtmpl(ind_ytmpl_ctr)-tdx_1;
% % % %         lane_rb=xtmpl(ind_ytmpl_ctr)+tdx_1;
% % % %         [~,~,strict]=InPolygon(pixX(:),pixY(:),[lane_lb'; flipud(lane_rb')],-[ytmpl_ctr'; flipud(ytmpl_ctr')]);
% % % %         ind_intensity=sub2ind([240,640],pixY(strict==1),pixX(strict==1));
% % % %         intensity_value=img(ind_intensity);
% % % %         n(ii,:)=hist(intensity_value,0:1:255);
        if (xtmpl2(end)-xtmpl(end))>0
        ...
        if ytmp_ll(1)>ytmp_rl(1)
            xtmp_ll(1:(ytmp_ll(1)-ytmp_rl(1)))=[];
            ytmp_ll(1:(ytmp_ll(1)-ytmp_rl(1)))=[];
        elseif ytmp_ll(1)<ytmp_rl(1)
            xtmp_rl(1:(ytmp_rl(1)-ytmp_ll(1)))=[];
            ytmp_rl(1:(ytmp_rl(1)-ytmp_ll(1)))=[];
        end
        if ytmp_ll(end)<ytmp_rl(end)
            xtmp_rl=[xtmp_rl, 640*ones(1,(ytmp_rl(end)-ytmp_ll(end)))];
            ytmp_rl=[ytmp_rl, ytmp_rl(end)-(1:(ytmp_rl(end)-ytmp_ll(end)))];
        elseif ytmp_ll(end)>ytmp_rl(end)
            xtmp_ll=[xtmp_ll, 1*ones(1,(ytmp_ll(end)-ytmp_rl(end)))];
            ytmp_ll=[ytmp_ll, ytmp_ll(end)-(1:(ytmp_ll(end)-ytmp_rl(end)))];
        end
        ind=(ytmp_rl>-100);
        xtmp_rl(ind)=[]; ytmp_rl(ind)=[];
        xtmp_ll(ind)=[]; ytmp_ll(ind)=[];
        newimg=zeros(240,640);
         for row_ind=1:length(ytmp_rl)
            inddel=abs(round(xtmp_rl(row_ind))-round(xtmp_ll(row_ind)))+1;
            exp_ind=1+round(((1:inddel)-1)*639/(inddel-1));
            newimg(-ytmp_rl(row_ind),exp_ind)=img(-ytmp_rl(row_ind),round(xtmp_ll(row_ind)):round(xtmp_rl(row_ind)));
            for jj=1:(inddel-1)
                if (exp_ind(jj+1)-exp_ind(jj))>1
                    fil_ind=(exp_ind(jj)+1):(exp_ind(jj+1)-1);
                    fil_val=(newimg(-ytmp_rl(row_ind),exp_ind(jj+1))-newimg(-ytmp_rl(row_ind),exp_ind(jj)))/(1+length(fil_ind));
                    fil_val=newimg(-ytmp_rl(row_ind),exp_ind(jj+1))+(1:length(fil_ind))*fil_val;
                    newimg(-ytmp_rl(row_ind),fil_ind)=fil_val;
                end
            end
...
%         for row_ind=1:length(ytmp_rl)
%             inddel=abs(round(xtmp_rl(row_ind))-round(xtmp_ll(row_ind)))+1;
%             rpt_num=floor(640/inddel);
%             for jj=1:rpt_num
%                 newimg(-ytmp_rl(row_ind),(1:inddel)+(jj-1)*inddel)=img(-ytmp_rl(row_ind),round(xtmp_ll(row_ind)):round(xtmp_rl(row_ind)));
%             end
%             newimg(-ytmp_rl(row_ind),(jj*inddel+1):640)=img(-ytmp_rl(row_ind),round(xtmp_ll(row_ind))-1+(1:(640-jj*inddel)));
%             newimg(-ytmp_rl(row_ind),round(xtmp_ll(row_ind)):round(xtmp_rl(row_ind)))=img(-ytmp_rl(row_ind),round(xtmp_ll(row_ind)):round(xtmp_rl(row_ind)));
%         end
...
%                 (Predict_Xc-xc_m).^2/var_w_xc+(Predict_d_theta-d_theta_m).^2/var_w_d_theta));
    ...
    [Estimate_xc, Estimate_d_theta]=solve_xc_d_theta(pre_abc,Estimate_x_mean,Estimate_z_mean,Estimate_theta_mean);
    if pre_leftrightlane~=leftrightlane
        if pre_leftrightlane==0
