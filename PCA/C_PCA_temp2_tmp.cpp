...
{
	double *ImgPtr, *win_sizePtr, *eigPtr, *coePtr, *pca_meanPtr, *roiPtr;
	...
	for (i=0; i<mrows*ncols; i++)
	{
			...
			&& r%2==0 && c%2==0 && *(roiPtr+i)>30)
		{
			...
			cent_r=cent_r/(*(roiPtr+i));
			cent_c=cent_c/(*(roiPtr+i));
			...
			*/
			//if (win_counter>pixel_num) {
				if (abs(34-cent_r)<=5 || abs(41-cent_c)<=5)
				{
