...
for outloop=1:1
    ...
    try
        ...
        for ii=(ite_s+1):ite_e
            ...
            [Btmp, Btmp2]=C_PCA(B,[63 82],u(:,1:20),g(1:20,:),img_pca_mean);
            log_minB(ii)=min(Btmp(:));
            [rr, cc]=find(Btmp==log_minB(ii));
% %             tic;
% %             [cimtmp,r,c]=harris(B,1,2.0,3,0);
% %             logt(ii)=toc;
            if log_minB(ii)<50
            imshow(B,[]); hold on; plot(cc(1),rr(1),'r.'); hold off; drawnow;
            frame=getframe(gcf);
            im=frame2im(frame);
