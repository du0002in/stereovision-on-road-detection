...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	plhs[3]=mxCreateDoubleMatrix(1,ncolsl,mxREAL);
    double *line_in_l=mxGetPr(plhs[3]);
	plhs[4]=mxCreateDoubleMatrix(1,ncolsr,mxREAL);
    double *line_in_r=mxGetPr(plhs[4]);
	plhs[5]=mxCreateDoubleMatrix(5,1,mxREAL);
	double *line_para_ptr=mxGetPr(plhs[5]);
    ...
    for(j=0;j<num_ite;j++)
    {
		...
		*(u+3)=*(SubDatapointr+2*rand1+1);
		...
		//*v=191; *(v+1)=-123; *(v+2)=171; *(v+3)=-105;
		//*u=10; *(u+1)=294; *(u+2)=36; *(u+3)=333;
		...
		#pragma omp critical
		{
			...
			if (tcon1>tcon_pre_l) {
				tcon_pre_l=tcon1;
				line_para_ptr[0]=c; line_para_ptr[1]=eleft; line_para_ptr[2]=fleft; line_para_ptr[3]=eright; line_para_ptr[4]=fright;
				for (int i=0;i<ncolsl;i++)
					*(line_in_l+i)=*(line_ptr_l1+i);
				for (int i=0;i<ncolsr;i++)
					*(line_in_r+i)=*(line_ptr_r1+i);
			}
		}
		delete [] line_ptr_l1;
		delete [] line_ptr_l;
