...
ini_xc=-1.0; ini_d_theta=-0.0863; %These two values need to guarantee that the line is the road center line not the road boundary line
map_y=(-1:0.01:100)'; map_x=-(tan(ini_d_theta)*map_y+ini_xc/cos(ini_d_theta));
map_z=zeros(length(map_x),1);
figure;plot(map_x,map_y); hold on; plotCar(pi/2,0,0,0); hold off;axis equal;
...
[leftimg, rightimg]=stereo_img_SL(0,0,pi/2,phi,H,dist_left_right,L_wheel_cam,Tpix2cam_left, Tpix2cam_right,map_x,map_y,map_z, Lane_marking_point_x, Lane_marking_point_y, Lane_marking_point_z);
% VehicleSimulator('Ini',ini_xc, ini_d_theta);
KickOff_PF_Simulator;
