...
if isempty(IDX)
    [IDX,xc]=knnsearch([map_x map_y],[X,Y]);
end
IDX=ind_xy(IDX);
...
elseif IDX==1
    gz=atan((map_z(IDX+1)-map_z(end))/sqrt((map_x(IDX+1)-map_x(end))^2+(map_y(IDX+1)-map_y(end))^2));
    theta_d=atan2(map_y(IDX+1)-map_y(end),map_x(IDX+1)-map_x(end));
else
