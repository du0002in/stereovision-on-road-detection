...
*MATLAB command window. */
 unsigned __stdcall SecondThreadFunc( void* pArguments ) {
	 ...
	 v_t=0; phi_t=0; dt=0; phi_a0=0; phi_a1=0; phi_a2=0;
	 v_a0=0; v_a1=0; v_a2=0;
	 theta = M_PI * 0.5; x=0; z=0;
	 ...
	 if ( (fp_SMC_log = fopen ("D:\\Stereo\\Vehicle Simulator\\ActualPosition_log.txt","w")) != NULL ) {
		 fprintf (fp_SMC_log, "dt x z theta phi_a v_a new_xc new_d_theta\n");
		 fclose (fp_SMC_log);
	 }
	 start_t = clock();
     while ( flag == 1 ) {
		 ...
		 if ( (fp_manuver = fopen ("D:\\Stereo\\manuver.txt", "r")) != NULL ) {
			 if ( (fscanf (fp_manuver, "%s %lf %s %lf", str1, &v_t, str2, &phi_t)) != 0 ) {
					...
					v_a0=(8.15*dt*dt*v_t+(2+2.064*dt)*v_a1-v_a2)/(1+2.064*dt+7.818*dt*dt);
					if (v_a0<0) v_a0=0;
					v_a2=v_a1; v_a1=v_a0;
					...
					if ( (fp_SMC_log = fopen ("D:\\Stereo\\Vehicle Simulator\\ActualPosition_log.txt","a")) != NULL ) {
						fprintf(fp_SMC_log, "%6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f %6.4f\n", dt, x, z, theta, phi_a0, v_a0, new_xc, new_d_theta);
						fclose (fp_SMC_log);
					}
			 }
			fclose(fp_manuver);
