...
*MATLAB command window. */
 unsigned __stdcall SecondThreadFunc( void* pArguments ) {
     ...
     while ( flag == 1 ) {
		 if ((( fp_v_m = fopen ("D:\\Stereo\\v_m.txt", "r")) != NULL) & (( fp_phi_m = fopen ("D:\\Stereo\\phai_m.txt", "r")) != NULL )) {
			 if ( (fscanf (fp_v_m, "%lf", &v_m) != 0) & (fscanf (fp_phi_m, "%lf", &phi_m) != 0) ) {
				 pre_v_m=v_m;
				 pre_phi_m=phi_m;
				 }
			 else {
