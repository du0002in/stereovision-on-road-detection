...
save_dir='D:\Stereo\realrun\test4';
...
%     rightimg=imread(strcat(rightimg_dir,int2str(ii),'.jpg'));
    ...
    dumy1=maxconvimg;
    dumy1(dumy1<connect_pixel*th)=0; dumy1(dumy1>=connect_pixel*th)=1;
    % tic;
    ...
    % toc
    dumy2=maxconvimg;
    dumy2(dumy2<connect_pixel*th)=0;dumy2(dumy2>=connect_pixel*th)=1;
    dumy=[dumy1;dumy2];
...
% %     sendtarget(v_t, phi_t);
    inner_thread('Con',Estimate_xc,Estimate_d_theta,pre_abc,L_wheels,Q_s,P_s,phi_d,v_t);
    if v_m~=0
