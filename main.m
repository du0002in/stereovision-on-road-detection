...
warning off;
f='C:\Users\a0107257\Desktop\Stereovision on road detection (github)\Image Collection(new Cam)91';
...
% log_ite_e=[386 812 378 530];
%for Image Collection(new Cam)58 at daytime
...
%     log_World{ii}=World;
    ...
    if sum(paraz(:)==0)
        if sum(abs(para(:,2)))==0 %model is hypobola
            ...
            Btmp=[Xw pre_Xw]';
            Wtmp=diag([ones(length(Zw),1); ones(length(pre_Zw),1)*01.0]);
