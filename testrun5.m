...
% leftimg_dir='D:\Stereo\Image Collection(new Cam)7\left';
% rightimg_dir='D:\Stereo\Image Collection(new Cam)7\right';
% save_dir='D:\Stereo\Image Collection(new Cam)7';
leftimg_dir='D:\Stereo\RealRun_Laptop_new_Cam\RealRunImg\test22\left';
rightimg_dir='D:\Stereo\RealRun_Laptop_new_Cam\RealRunImg\test22\right';
save_dir='D:\Stereo\RealRun_Laptop_new_Cam\RealRunImg\test22\Offline';
...
% log_ite_s=[39 371 691 1490];
% log_ite_e=[217 461 798 1540];

%for Image Collection(new Cam)2,3,4
log_ite_s=[1 1 1 1 55 25]; 
log_ite_e=[160  247 100 2040 307 200];
...
miss_infor_flag=0;
Phai0=0.3111;
D_pre=focal*tan(Phai0);H=1.665;marking_width_pre=0.10;diff_marking_width=0;lane_width=3.0;
...
%     dumy2(dumy2<connected_pixel*th)=0;dumy2(dumy2>=connected_pixel*th)=1;
    ...
    % dumy(:,end-320:end)=0;
    imwrite(dumy,strcat(save_dir,'\oriRidge',int2str(ii),'.jpg'),'jpg');
...
%     log_Yw_mean(ii)=mean(Yw);
    ...
    if sum(ind_near)>=5
        ...
        makring_width=0.1; %fixed for demo
        if leftrightlane==0
