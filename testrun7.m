...
log_ite_s=[1 263 460 1490];
log_ite_e=[100 382 648 1540];
...
% % % % %     end
    ...
    if sum(paraz(:)==0)
        [sel_xtmpl2,sel_ytmpl2,xtmpl2, ytmpl2, sel_xtmpr2,sel_ytmpr2,xtmpr2, ytmpr2, num_in2, para2, lwtmp, dumytmp, xtmpl_imagine, xtmpr_imagine, ytmp_imagine]= ...
            fit_other_lane(leftrightlane,dumy1,refine_para,-ytmpr(1),-ytmpr(end),H,Phai,D_pre);
        if isempty(xtmpl2)
            if leftrightlane==0
        ...
        [xtmpl2,ytmpl2,xtmpr2,ytmpr2,num_in2,dumytmp]=fit_other_lane_zebra(leftrightlane,dumy1,xtmpl,ytmpl,xtmpr,ytmpr,D_pre,H);
        if isempty(xtmpl2)
            if leftrightlane==0
