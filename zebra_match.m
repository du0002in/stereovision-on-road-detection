...
%     for ii=[135:143, 284:287, 579:586, 733:740]
    for ii=ite_s:ite_e
        ...
        startrow=1; endrow=1;
        for row_z=1:size(B,1)
            %         for row_z=true_s_r:true_e_r
            if row_z==29 || row_z==132
                aa=0;
            end
            b=B(row_z,:);
            cum_b=cumsum(b);
            ...
            if cum_b(end)~=0 && (end0_ind-lead0_ind>=25)
                ...
                if (fre_seq(ma_ind)>=4 && fre_seq(ma_ind)<=6.5) && (ma_max>=7) && (zebra_status==1)
                    ...
                    % %                     if row_z>=true_s_r && row_z<=true_e_r
                    % %                         TP_counter=TP_counter+1;
                    % %                     else
                    % %                         FP_counter=FP_counter+1;
                    % %                     end
                    %                     hold on;plot(fre_seq(1:10),ma(1:10));hold off;
                elseif ((end0_ind-(lead0_ind+15))>=25)
                    ...
                    if (fre_seq(ma_ind)>=4 && fre_seq(ma_ind)<=6.5) && (ma_max>=7) && (zebra_status==1)
                        ...
                        %                         counter=counter+1;
                        %                         log_abnorm(counter,:)=[ii, row_z, ma_max, ma_max2];
                    end
                end
                %                 end
            end
